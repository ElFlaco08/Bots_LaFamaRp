...
const commands = [
    ...
    new SlashCommandBuilder().setName('player').setDescription('Nombre Jugadores'),
]
...
cloodybot.once('ready', () => {
    ...
    setInterval(() => {
        cloodymta.query({
            ...
            cloodybot.user.setActivity(`LFMRP : ${state.raw.numplayers}/${state.maxplayers}`);
        }).catch(err => {
            console.log(err);
        });
    }, 5000);
...
cloodybot.on('interactionCreate', async cloodymsg => {
	...
	if (commandName === 'server') {
		cloodymta.query({
            ...
            .addField(`Developer :`,` - WelinCode`,true)
...
cloodybot.on('interactionCreate', async cloodymsg => {
    ...
    players.push(`${p.name}\n`)
            ...
            .setColor(`BLUE`)
            .addFields(
                { name: 'Nombre de Jugadores', value: `\`\`\`${players}\`\`\``, inline: true },   
                )
